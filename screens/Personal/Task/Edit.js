...
const TaskEdit = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    else setTaskData({ taskDueDate: moment().format(), taskDetails: '', taskStatus: 3 });
  }, []);
  ...
  const onChangeDate = (event, selectedDate) => {
    ...
    if (selectedDate) onChange('taskDate', selectedDate.toISOString());
  };
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    personalAPI.editTask(param)
      .then(response => {
        if (response.result) {
          navigation.goBack();
        } else {
          Alert.alert( `${task ? 'Update' : 'Create'} a task failed`);
        }
      ...
      <ScrollView contentContainerStyle={styles.scrollViewContainer} style={styles.scrollView}>
        <View style={styles.centeredView}>
          ...
          <View style={styles.section}>
            ...
            <TouchableHighlight style={styles.dateView} onPress={() => setShowPicker(true)}>
              <View style={styles.inputView}>
                <Text style={styles.textInput}>{taskData && taskData.taskDueDate}</Text>
              </View>
            </TouchableHighlight>
            {showPicker && (
              <DateTimePicker
                testID="dateTimePicker"
                value={(taskData ? moment(taskData.taskDueDate) : moment()).toDate()}
                mode="datetime"
                display="default"
          ...
          <View style={styles.section}>
            ...
            <DropDownPicker
              items={statusItems}
              defaultValue={taskData ? taskData.taskStatus : 3}
              containerStyle={{ height: 50 }}
              style={{ backgroundColor: '#e1ebf9' }}
              itemStyle={{ justifyContent: 'flex-start' }}
              dropDownStyle={{ backgroundColor: '#e1ebf9' }}
              onChangeItem={item => onChange('taskStatus', item.value)}
            />
          </View>
          <View style={styles.section}>
            <View style={styles.sectionTitleView}>
            ...
            <View style={styles.textAreaView}>
              <TextInput
                ...
                value={taskData && taskData.taskDetails}
                onChangeText={value => onChange('taskDetails', value)}
              />
            </View>
          </View>
